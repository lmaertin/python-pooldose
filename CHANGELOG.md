...
- Fixed #54: aligned `PDRX1H1HAW100` firmware `539176` with the real-device
  payloads and exposed its ORP-only entity set through the
  `PDPH1H1HAW100` mapping alias.
- Corrected faulty `0.9.12` support for the ORP-only `PDRX1H1HAW100` model.
- Added ORP measurement, setpoint, dosing, calibration, and alarm mappings
  together with the shared operational status, delay, relay, and unit fields.
...
- Moved the dedicated mapping-file existence check off the event loop while
  loading an aliased model mapping.
- Added regression coverage to ensure the check continues to run in a worker
  thread.
...
- Added mapping support for BWT MEDO CONNECT Wi-Fi (PDPH1H1HAW1B0).
- Improved missing-mapping handling with clearer warning text and a new RequestStatus value.
...
## Changed
* Add PDHC1H1HAR1V0 alias for VÁGNER POOL VA DOS BASIC by @ryja81 in https://github.com/lmaertin/python-pooldose/pull/43

### Contributors

Thanks to [@ryja81](https://github.com/ryja81) for contributing to this release.
